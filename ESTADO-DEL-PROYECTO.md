...
| `SEC-14` | 🟠 Media | **Next.js 16.1.6 acumula 28 advisories.** `npm audit` reporta 13 vulnerabilidades (8 altas), todas **preexistentes** — vienen de `next` y de la cadena de `@ducanh2912/next-pwa` (workbox, rollup, babel, serialize-javascript), no del tooling añadido en la rama 0. **Rebajada de 🔴 a 🟠 el 2026-07-27:** los tres bypass de middleware (GHSA-26hh-7cqf-hhc6, GHSA-267c-6grr-h53f, GHSA-492v-c6pp-mqqv) eran lo que la hacía crítica, porque `proxy.ts` era la única protección de `/admin/metrics` con `service_role`. Borrado ese subsistema, **ya no hay middleware ni blanco privilegiado**. Sigue pendiente por lo demás: SSRF en rewrites, XSS con nonces de CSP, cache poisoning de RSC, DoS en la API de optimización de imágenes. **Fix sin cambio mayor: `next@16.2.12`.** También `ws`, `fast-uri` y `brace-expansion` tienen fix directo. Detectado 2026-07-27. | [ ] |
...
- [ ] `SEC-14` Subir `next` a 16.2.12 (sin cambio mayor) + `npm audit fix` de `ws`,
      `fast-uri` y `brace-expansion`. Cierra los bypass de middleware que hacen explotable
      el `service_role` de `/admin/metrics`. **Añadido al bloque el 2026-07-27**; va con el
      gate `npm run verify` + build, porque un salto de minor de Next puede romper el build.
