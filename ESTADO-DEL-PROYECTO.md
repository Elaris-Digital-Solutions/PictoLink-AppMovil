...
- **Última actualización:** 2026-08-29 (`E.0.2`: el CI **corre en verde**, verificado en el log y no por el tick; falta el control negativo. `SEC-16` **contenida**: los 5 PAT revocados, HTTP 401 en los cinco)
...
- [x] CI en GitHub Actions ~~← pendiente de decisión del usuario~~ **Desbloqueado por `DEC-7`**
      ...
      log: typegen OK, lint 88, tests 13+1 todo, humo 27/27. Sigue faltando el control negativo
      (un PR con un error de lint a propósito tiene que salir rojo). **El rojo del 2026-08-29 no lo
      cubre:** lo disparó `Typecheck`, con `Lint` en verde antes, así que nunca se ha visto al paso
      `Lint` cortar un job
