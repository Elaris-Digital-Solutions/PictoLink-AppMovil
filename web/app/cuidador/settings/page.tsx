...
/**
 * /cuidador/settings — Ajustes del Cuidador
 *
 * Accessible from the caregiver interface header.
 * Provides TTS voice / rate settings and sign-out.
 * Lives under /cuidador/* so AppShell's route guard allows it.
 */

import { useState, useMemo } from 'react';
import { useRouter } from 'next/navigation';
import { ArrowLeft, Volume2, Check, Play, Languages, LogOut, UserCircle } from 'lucide-react';
...
export default function CuidadorSettingsPage() {
    ...
    return (
        <div className="flex flex-col h-dvh bg-[#FFF7F2]">
            {/* Header */}
            <div className="flex-shrink-0 flex items-center gap-3 px-4 pt-5 pb-4 bg-[#FFF4ED] border-b border-[#FFD5BF]">
                <button
                    onClick={() => router.back()}
                    className="p-2 -ml-1 text-[#C85F27] hover:bg-[#FFE6D6] rounded-full transition-colors"
                    aria-label="Volver"
                >
                    <ArrowLeft size={22} />
                </button>
                <div>
                    <h1 className="text-xl font-black text-[#FF8844] leading-none">Ajustes</h1>
            ...
            <div className="flex-1 overflow-y-auto py-4">
                ...
                <div className="mb-6">
                    <div className="flex items-center gap-2 px-4 py-2 mb-1">
                        <UserCircle size={15} className="text-[#C85F27]" />
                        <h2 className="text-[11px] font-black text-[#C85F27] uppercase tracking-widest">Foto de perfil</h2>
                    </div>
                        ...
                        <div className="flex items-center gap-4 px-5 py-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-400 mt-0.5">
                                    {profile?.avatar_url ? 'Toca para cambiar la foto' : 'Sin foto — toca para agregar'}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="mb-6">
                    <div className="flex items-center gap-2 px-4 py-2 mb-1">
                        <Volume2 size={15} className="text-[#C85F27]" />
                        <h2 className="text-[11px] font-black text-[#C85F27] uppercase tracking-widest">Voz y habla</h2>
                    </div>
                    <div className="bg-white border border-[#FFE2D0] rounded-2xl overflow-hidden mx-3 shadow-sm">
                        ...
                        {/* Voice picker */}
                        {showVoicePicker && (
                            <div className="border-b border-[#FFF0E8] max-h-64 overflow-y-auto">
